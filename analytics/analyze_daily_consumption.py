...
def analyze_daily_consumption():
    ...
    # Additional analysis
    print(f"\nAdditional insights:")
    print(f"Total consumption period: {daily_consumption.sum():.2f} Wh ({daily_consumption.sum()/1000:.2f} kWh)")
    print(f"Average hourly consumption: {daily_consumption.mean()/24:.2f} Wh")
    
    # Day of week analysis
    df["day_of_week"] = df["time"].dt.day_name()
    daily_avg_by_weekday = df.groupby("day_of_week")["watt_hours"].sum().groupby(level=0).mean()
...
if __name__ == "__main__":
    daily_consumption = analyze_daily_consumption()
